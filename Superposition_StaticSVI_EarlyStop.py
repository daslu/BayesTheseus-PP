...
class SVIEngine(Engine):
    def __init__(self, *args, step_args=None, **kwargs):
        ...
        self.svi = StaticSVI(*args, **kwargs)
...
def model(data):
    ...
    ### 1. prior over mean M
    M = pyro.sample("M", dist.StudentT(0.01,0, max_var).expand_by([data1.size(0),data1.size(1)]).to_event(2))
    M = Center_torch(M)
    ...
    #ri_vec = pyro.sample("ri_vec",dist.Normal(0,0.5).expand_by([3]).to_event(1))  # Weak normal? wih torch.sigmoid
    R = sample_R(ri_vec)
    M_T1 = M + T1
    R = sample_R(ri_vec)
    M_R2_T2 = M @ R + T2
    ...
    with pyro.plate("plate_univariate", data1.size(0)*data1.size(1),dim=-1):
        pyro.sample("X1", dist.StudentT(0.05,M_T1.view(-1), U),obs=data1.view(-1))
        pyro.sample("X2", dist.StudentT(0.05,M_R2_T2.view(-1), U), obs=data2.view(-1))


def Run(data_obs,average,name):
    #GUIDE
    global_guide = AutoDelta(model)
    optim = pyro.optim.LBFGS({'lr':0.03})
    ...
    def update_progress(svi_engine):
        ...
        #     print(param_list)
    svi_engine.run([data_obs],max_epochs=1) #max_epochs=3

    # Register hooks to monitor gradient norms.
    gradient_norms = defaultdict(list)
    for name_i, value in pyro.get_param_store().named_parameters():
        value.register_hook(lambda g, name=name: gradient_norms[name_i].append(g.norm().item()))
    handler = EarlyStopping(patience=120, score_function=lambda engine: engine.state.output, trainer=svi_engine)
    #SVI
    svi_engine.add_event_handler(Events.EPOCH_COMPLETED, handler)
    svi_engine.run([data_obs],max_epochs=90000)
    #PLOTTING ELBO
    #plt.figure(1)
    plt.plot(loss_list)
