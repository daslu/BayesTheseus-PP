...
class SuperpositionModel():
    ...
    def guide(self,data):
        '''Guide contains the priors initializations to approximate the full posterior'''
        #https://github.com/pyro-ppl/pyro/blob/f9aca3c484266a385a8ff01f7a2f14ee2e549d14/tests/infer/test_abstract_infer.py
        max_var,data1, data2 = data
        ### 1. prior over mean M:
        #Hyperparameters
        df = pyro.param("df",torch.Tensor([1.]))
        mu_M = pyro.param("mu_M",torch.Tensor([0.]))
        sd_M= pyro.param("sd_M",torch.Tensor([3.]))
        pyro.sample("auto_M", dist.StudentT(df,mu_M, sd_M).expand_by([data1.size(0),data1.size(1)]).to_event(2))
        ### 2. Prior over variances for the normal distribution
        mu_U =pyro.param("mu_U",torch.Tensor([1.]))
        pyro.sample("auto_U", dist.HalfNormal(mu_U).expand_by([data1.size(0)]).to_event(1))
        #U =  pyro.param("",U.reshape(data1.size(0),1).repeat(1,3).view(-1) ) #Triplicate the rows for the subsequent mean calculation
        ## 3. prior over translations T_i: Sample translations for each of the x,y,z coordinates
        mu_T2 = pyro.param("mu_T2",torch.Tensor([0.]))
        sd_T2 = pyro.param("sd_T2",torch.Tensor([1.]))
        pyro.sample("auto_T2", dist.Normal(mu_T2, sd_T2).expand_by([3]).to_event(1))
        ## 4. prior over rotations R_i
        mu_ri = pyro.param("mu_T2", torch.Tensor([0.]))
        sd_ri = pyro.param("sd_T2", torch.Tensor([1.]))
        pyro.sample("auto_ri_vec",dist.Uniform(mu_ri, sd_ri).expand_by([3]).to_event(1))  # Uniform distribution
        #R = self.sample_R(ri_vec)
        #M_T1 = M
        #M_R2_T2 = M @ R + T2
    def Run(self,data_obs,average,name,):
        #GUIDE
        global_guide = self.guide(data_obs)
        #OPTIMIZER
