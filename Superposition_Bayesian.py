...
def Pymol_Samples(data1,data2,name1,R_samples,T_samples,samples):
    ...
    for i in range(0,samples):
        ...
        write_ATOM_line(X2, os.path.join("{}_PDB_files".format(name1),'Result_MCMC_{}_X2_{}.pdb'.format(name1,i)))
    ...
    plt.close()
    names = [os.path.join("{}_PDB_files".format(name1),'Result_MCMC_{}_X2_{}.pdb'.format(name1,i)) for i in indexes] #exchange indexes with range(0,samples)
    Pymol(*names)
...
Folders("{}_PDB_files".format(name1))
data_obs = Read_Data('../PDB_files/{}.pdb'.format(name1), '../PDB_files/{}.pdb'.format(name2),type='chains',models =models,RMSD=True)
