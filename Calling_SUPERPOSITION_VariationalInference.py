...
from Superposition_VariationalInference import *
import torch
import time
import pandas as pd
import timeit
import torch
name1 = '2cpd' #2nl7:139 #2do0=114 #2cpd=99
name2 ='2cpd'
...
iterations=10
samples = 10 #Apparently AutoDiagonalNormal only takes 10 samples
...
for i in range(iterations):
    ...
    #torch.manual_seed(random_with_N_digits(19))
    torch.manual_seed(10+i)
    #Create a folder to store the PDB files
    DataManagement.Folders("{}_PDB_files".format(name1))
    #Read the data, calculate distance of the structure to the origin and the average structure between the inputs
    data_obs = DataManagement.Read_Data('../PDB_files/{}.pdb'.format(name1), '../PDB_files/{}.pdb'.format(name2),type='models',models =(0,2),RMSD=True)
    ...
    T2, R, M, X1, X2,distances,info,duration,ri_vec_samples,M_samples,T_samples = SuperpositionModel.Run(data_obs, average, name1,samples)
    ...
    DataManagement.Pymol("Result_{}_X1.pdb".format(name1))
    ...
    DataManagement.Pymol_Samples(data1, data2, name1, ri_vec_samples, T_samples, samples)
