...
def Run(data_obs,average,name,):
    ...
    def update_progress(svi_engine):
        ...
        #pbar.set_description("[epoch {}] avg train loss: {}
    ...
    handler = EarlyStopping(patience=250, score_function=lambda engine: engine.state.output, trainer=svi_engine)
    #SVI
    ...
    return T1.numpy(),T2.numpy(),R.numpy(),M, X1,X2,distances,(svi_engine.state.epoch,svi_engine.state.output),duration
